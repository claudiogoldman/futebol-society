...
replace(`      avatar: g.avatar || '⚽',`, `      avatar: g.avatar || null,
      avatarUrl: g.avatar_url || '',`, 'group avatar mapping');

replace(`  const [avatarDraft, setAvatarDraft] = useState(group.avatar || '⚽');`, `  const [avatarDraft, setAvatarDraft] = useState(group.avatar || null);
  const [avatarUrlDraft, setAvatarUrlDraft] = useState(group.avatarUrl || '');
  const [avatarUploading, setAvatarUploading] = useState(false);`, 'group avatar state');

replace(`  const isOwner = myId === group.createdBy;`, `  const isOwner = myId === group.createdBy;
...
replace(`      avatar: avatarDraft,`, `      avatar: avatarDraft,
      avatar_url: avatarUrlDraft || null,`, 'group avatar save');

replace(`            <div className="sf-cost-row"><span className="sf-muted">Avatar</span><span className="sf-mono-value" style={{ cursor: 'default' }}>{group.avatar || '⚽'}</span></div>`, `            <div className="sf-cost-row"><span className="sf-muted">Imagem do grupo</span><img src={group.avatarUrl || '/group-avatars/futebol.svg'} alt="Imagem do grupo" style={{ width: 42, height: 42, objectFit: 'cover', borderRadius: 10, border: '1px solid var(--sf-border)' }} /></div>`, 'group avatar display');

replace(`            {isOwner && <button className="sf-btn-ghost" style={{ width: '100%', marginTop: 6 }} onClick={() => setEditing(true)}>Editar padrões</button>}`, `            {canManage && <button className="sf-btn-ghost" style={{ width: '100%', marginTop: 6 }} onClick={() => setEditing(true)}>Editar padrões</button>}`, 'group management button');

replace(`            <label className="sf-field-label">Avatar do grupo</label>
            ...
            </div>`, `            <label className="sf-field-label">Imagem do grupo</label>

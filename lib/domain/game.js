...
function teamScore(team, player, sumRating, sumPhysical) {
  const rating = Number(player?.rating) || 3;
  const physical = physicalScore(player);
  const projectedRatingGap = Math.abs(sumRating + rating);
  const projectedPhysicalGap = Math.abs(sumPhysical + physical);
  const coverage = positionFamilyScore(team, player);

  // Coverage matters, but substantially less than competitive balance. This
  // prevents a missing position from forcing an otherwise poor split.
  return coverage * 0.8 - projectedRatingGap * 0.02 - projectedPhysicalGap * 0.01;
}
...
export function drawTeams(confirmedPlayers = [], random = Math.random, config = {}) {
  ...
  const place = (player, preferredTeam = null) => {
    ...
    if (preferredTeam === 'A' && teamA.length < teamCapacity) {
      ...
      const scoreA = teamScore(teamA, player, ratingGap, physicalGap);
      const scoreB = teamScore(teamB, player, -ratingGap, -physicalGap);

...
export function drawTeams(confirmedPlayers = [], random = Math.random, config = {}) {
  ...
  outfieldPlayers
    .map((player) => {
      ...
      return { player, noisyRating: (Number(player.rating) || 3) + random() * 0.5, rarity };
    })
...
/** Compare the starting squads and return a normalized 0-100 balance index. */
export function calculateTeamBalance(teamA = [], teamB = []) {
  ...
  const average = (players, selector) => players.length
    ? players.reduce((sum, player) => sum + selector(player), 0) / players.length
    : 0;
  ...
  const classification = balance >= 95 ? 'Excelente equilíbrio'
    : balance >= 90 ? 'Muito bom'
    : balance >= 80 ? 'Bom'
    : balance >= 70 ? 'Atenção'
    : 'Desequilibrado';

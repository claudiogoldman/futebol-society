...
function teamScore(team, player, ownRating, otherRating, ownPhysical, otherPhysical, ownRanking, otherRanking, ownWins, otherWins, ownGoals, otherGoals, ownAssists, otherAssists, ownEval, otherEval, weights) {
  ...
  const goals = Number(player?._goals) || 0;
  const assists = Number(player?._assists) || 0;
  const evaluation = Number(player?._rating) || 0;
  ...
  const projectedGoalsImbalance = Math.abs((ownGoals + goals) - otherGoals);
  const projectedAssistsImbalance = Math.abs((ownAssists + assists) - otherAssists);
  const projectedEvalImbalance = Math.abs((ownEval + evaluation) - otherEval);
  return positionFamilyScore(team, player) * 0.55
    - projectedRankingImbalance * weights.ranking
    - projectedWinsImbalance * weights.wins
    - projectedGoalsImbalance * weights.goals
    - projectedAssistsImbalance * weights.assists
    - projectedEvalImbalance * weights.evaluation
...
function applyReserveOrder(team, reservesPerTeam, playersPerTeam) {
  ...
  // A ordem de inscrição continua definindo as reservas, mas nunca remove
  // um goleiro dos titulares. Goleiros são sempre preservados como titulares
  // quando há vaga de titular no time.
  ...
  const reserveCandidates = ordered.filter((player) => !isGoalkeeper(player));
  const reserves = reserveCandidates.slice(-reserveCount);
...
/**
 * O índice de equilíbrio considera o elenco completo:
 * titulares + reservas.
 */
function balanceRoster(team = []) {
  // Titular/reserva é apenas um papel. O índice mede o elenco, então cada
  // jogador deve entrar exatamente uma vez no cálculo, independentemente da
  // forma como o estado foi montado ou remanejado.
  const seen = new Set();
...
export function teamMetrics(team = []) {
  const starters = balanceRoster(team);
...
export function calculateTeamBalance(teamA = [], teamB = [], config = {}) {
  // Penalidade configurável, em pontos de força, aplicada quando o time não tem goleiro.
  // O valor padrão preserva uma penalização explícita de 10 pontos para o goleiro improvisado.
  const improvisedGoalkeeperPenalty = Number.isFinite(Number(config.improvisedGoalkeeperPenalty))
    ? Math.max(0, Math.min(30, Number(config.improvisedGoalkeeperPenalty)))
    : 10;
  const startersA = balanceRoster(teamA);
  const startersB = balanceRoster(teamB);
  ...
  const strength = (players) => {
    const hasGoalkeeper = goalkeeper(players) === 1;
    const base = (rating(players) * 0.60 + coverage(players) * 0.15 + (hasGoalkeeper ? 0.10 : 0)) * 100;
    return Math.max(0, base - (hasGoalkeeper ? 0 : improvisedGoalkeeperPenalty));
  };
  const strengthA = strength(startersA);
  const strengthB = strength(startersB);
  const goalkeeperStatusA = goalkeeper(startersA) ? 'titular' : 'improvisado';
  const goalkeeperStatusB = goalkeeper(startersB) ? 'titular' : 'improvisado';
  ...
  // O índice já está em pontos percentuais de diferença ponderada. Não
  // multiplicar novamente por 3: isso fazia diferenças relevantes saturarem
  // em 0/100 e destruía a gradação do índice.
  const balance = Math.max(0, Math.min(100, 100 - relativeDifference));
...
function drawTeamsOnce(confirmedPlayers = [], random = Math.random, config = {}) {
  const rawWeights = {
    ranking: Number.isFinite(Number(config.balanceRankingWeight)) ? Math.max(0, Number(config.balanceRankingWeight)) : 0.35,
    wins: Number.isFinite(Number(config.balanceWinsWeight)) ? Math.max(0, Number(config.balanceWinsWeight)) : 0.30,
    goals: Number.isFinite(Number(config.balanceGoalsWeight)) ? Math.max(0, Number(config.balanceGoalsWeight)) : 0.15,
    assists: Number.isFinite(Number(config.balanceAssistsWeight)) ? Math.max(0, Number(config.balanceAssistsWeight)) : 0.10,
    evaluation: Number.isFinite(Number(config.balanceRatingWeight)) ? Math.max(0, Number(config.balanceRatingWeight)) : 0.05,
  };
  const weightTotal = Object.values(rawWeights).reduce((sum, value) => sum + value, 0) || 1;
  const weights = Object.fromEntries(Object.entries(rawWeights).map(([key, value]) => [key, value / weightTotal]));
  const playersPerTeam = Math.max(1, Number(config.playersPerTeam) || Math.ceil(confirmedPlayers.length / 2));
  const configuredReservesPerTeam = Math.max(0, Number(config.reservesPerTeam) || 0);
  const reservesPerTeam = confirmedPlayers.length > playersPerTeam * 2 ? configuredReservesPerTeam : 0;
  ...
  let teamA = [], teamB = [], sumA = 0, sumB = 0, physA = 0, physB = 0, rankingA = 0, rankingB = 0, winsA = 0, winsB = 0, goalsA = 0, goalsB = 0, assistsA = 0, assistsB = 0, evalA = 0, evalB = 0;
  const place = (player, preferredTeam = null) => {
    ...
    if (preferredTeam === 'A' && teamA.length < teamCapacity) goesToA = true;
      ...
      const scoreA = teamScore(teamA, player, sumA, sumB, physA, physB, rankingA, rankingB, winsA, winsB, goalsA, goalsB, assistsA, assistsB, evalA, evalB, weights);
      const scoreB = teamScore(teamB, player, sumB, sumA, physB, physA, rankingB, rankingA, winsB, winsA, goalsB, goalsA, assistsB, assistsA, evalB, evalA, weights);
    ...
    const evaluation = Number(player?._rating) || 0;
    if (goesToA) {
      teamA.push(player); sumA += rating; physA += physical; rankingA += rankingPoints; winsA += wins; goalsA += goals; assistsA += assists; evalA += evaluation;
    } else {
      teamB.push(player); sumB += rating; physB += physical; rankingB += rankingPoints; winsB += wins; goalsB += goals; assistsB += assists; evalB += evaluation;
    }
  };
...
export function drawTeams(confirmedPlayers = [], random = Math.random, config = {}) {
  ...
  for (let i = 0; i < candidates; i += 1) {
    ...
    // O objetivo do sorteio usa o mesmo índice de equilíbrio do elenco completo.
    const balance = calculateTeamBalance(candidate.teamA, candidate.teamB, config);
    ...
    const goalsDifference = Math.abs(candidate.teamAStarters.reduce((s, p) => s + (Number(p?._goals) || 0), 0) - candidate.teamBStarters.reduce((s, p) => s + (Number(p?._goals) || 0), 0));
    const assistsDifference = Math.abs(candidate.teamAStarters.reduce((s, p) => s + (Number(p?._assists) || 0), 0) - candidate.teamBStarters.reduce((s, p) => s + (Number(p?._assists) || 0), 0));
    const evalA = candidate.teamAStarters.reduce((s, p) => s + (Number(p?._rating) || 0), 0);
    const evalB = candidate.teamBStarters.reduce((s, p) => s + (Number(p?._rating) || 0), 0);
    const evaluationDifference = Math.abs(evalA - evalB);
    const rawWeights = {
      ranking: Number.isFinite(Number(config.balanceRankingWeight)) ? Math.max(0, Number(config.balanceRankingWeight)) : 0.35,
      wins: Number.isFinite(Number(config.balanceWinsWeight)) ? Math.max(0, Number(config.balanceWinsWeight)) : 0.30,
      goals: Number.isFinite(Number(config.balanceGoalsWeight)) ? Math.max(0, Number(config.balanceGoalsWeight)) : 0.15,
    ...
    const weightTotal = Object.values(rawWeights).reduce((sum, value) => sum + value, 0) || 1;
    const weights = Object.fromEntries(Object.entries(rawWeights).map(([key, value]) => [key, value / weightTotal]));
    const objective = balance.relativeDifference + rankingDifference * weights.ranking + winsDifference * weights.wins + goalsDifference * weights.goals + assistsDifference * weights.assists + evaluationDifference * weights.evaluation;

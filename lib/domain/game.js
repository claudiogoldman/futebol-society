...
function rebalanceGoalkeepers(teamA, teamB) {
  ...
  // Best goalkeeper = lowest average goals conceded per completed match.
  // If there is no history for a goalkeeper, the draw cannot rank the player
  // defensively, so the existing order is preserved as the neutral fallback.
  ...
  // The weaker side is determined first by ranking points and then by wins.
  // Lower values mean the side has the weaker historical competitive record.
  ...
  const bestIsWeaker = weakerTeam.some((player) => player.id === bestGoalkeeper.id);
  if (bestIsWeaker) return;
...
function applyReserveOrder(team, reservesPerTeam) {
  if (!reservesPerTeam || !team.length) return { starters: [...team], reserves: [] };
  ...
  const reserveCount = Math.min(reservesPerTeam, Math.max(0, ordered.length - 1));
...
function drawTeamsOnce(confirmedPlayers = [], random = Math.random, config = {}) {
  ...
  // Keep one goalkeeper on each side when two are available. The final
  // goalkeeper correction below guarantees that the least-conceded keeper is
  // on the weaker side by ranking/wins.
  ...
  const splitA = applyReserveOrder(teamA, reservesPerTeam);
  const splitB = applyReserveOrder(teamB, reservesPerTeam);

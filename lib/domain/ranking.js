...
export function computeGameHighlights(game) {
  ...
  return {
    ...
    maxAssists,
    // Keep `muro` for backward compatibility with existing UI consumers.
    // `muros` is the authoritative collection for ranking/scoring.
    muro: muros[0] || null,
    muros,
    muroConceded: muros.length === 1
...
export function computeRanking(profiles = [], games = []) {
  ...
  completedGames.forEach((game) => {
    ...
    [...idsA, ...idsB].forEach((id) => {
      ...
      stats[id].jogos += 1;
      // Every completed participation contributes 1 ranking point.
      stats[id].pontos += 1;
    ...
    // The top scorer of the match gets 1 point. Goals themselves remain
    // statistical data and do not contribute points to the general ranking.
    ...
    // A Wall exists only when both teams have a goalkeeper. Each goalkeeper
    // who qualifies as a Wall receives 1 point.
  ...
  return Object.values(stats)
    ...
    // Goals are deliberately not used as a tie-breaker in the general ranking.

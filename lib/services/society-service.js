...
export async function addGameParticipant(gameId, userId) { return result(await supabase.from('game_confirmations').insert({ game_id: gameId, user_id: userId })); }
export async function removeGameParticipant(gameId, userId) { return result(await supabase.from('game_confirmations').delete().eq('game_id', gameId).eq('user_id', userId)); }

...
begin
  ...
  if v_confirmed_user_id is not null then
    insert into public.notifications (user_id, actor_id, type, title, body, game_id, group_id)
    values (v_next.user_id, null, 'game_waitlist_promoted', 'Você foi promovido da lista de espera', 'Uma vaga ficou disponível e sua presença foi confirmada automaticamente.', old.game_id, v_game.group_id);
  end if;

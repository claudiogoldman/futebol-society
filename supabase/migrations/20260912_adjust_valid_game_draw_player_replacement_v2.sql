-- Corrige o ajuste para operar sobre o sorteio explicitamente selecionado.
-- Não usa implicitamente o último sorteio inválido.
...
begin
  ...
  if not found then
    raise exception 'O sorteio selecionado não está disponível para ajuste.';
  end if;
  ...
  if not v_replaced then raise exception 'Jogador de saída não pertence ao sorteio selecionado.'; end if;
  ...
  if exists (select 1 from public.game_participants gp where gp.game_id = p_game_id and gp.user_id = p_out_user_id) then
    raise exception 'O jogador de saída ainda está vinculado à participação da partida.';
  end if;

  if not exists (select 1 from public.game_participants gp where gp.game_id = p_game_id and gp.user_id = p_in_user_id) then
    raise exception 'O jogador de entrada não está confirmado na partida.';
  end if;

  select coalesce(max(h.draw_number), 0) + 1 into v_new_draw_number
    from public.game_draw_history h where h.game_id = p_game_id;
  ...
  insert into public.game_draw_history (
    ...
    p_game_id, v_new_draw_number, auth.uid(), v_source.players_per_team, v_source.reserves_per_team,
    v_team_a_starters, v_team_b_starters, v_team_a_reserves, v_team_b_reserves, true
  ) returning id into v_new_draw_id;

  update public.game_draw_history
     set is_valid = false
   where game_id = p_game_id and id <> v_new_draw_id and is_valid = true;
